    ...
    return (
        <div className="card bg-secondary text-secondary-content">
            <div className="card-body gap-2 justify-between">
                ...
                <p className='badge badge-xs'>{(availability == "true")? "Available" : "Not Available"}</p>
                <p>{roomType}</p>
                <p>{location}</p>
                <p>Rent: {rent} Taka</p>
                </div>
                <div className="card-actions justify-end">
                    <button className="btn btn-xs md:btn-sm" onClick={()=> seeMoreBtnHandler(_id)}>See More</button>
                </div>
            </div>
        </div>
    );
